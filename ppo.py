...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        action_alpha = F.softplus(self.actor_alpha(x)+1)
        action_beta = F.softplus(self.actor_beta(x)+1)
        probs = torch.distributions.Beta(action_alpha, action_beta)
        if action is None:
            action = probs.sample()
...
if __name__ == "__main__":
    ...
    run_name = run_name + "smallClipParam/seeds/27"
    if args.track:
        import wandb
    ...
    writer = SummaryWriter(f"runsTemp/{run_name}")
    writer.add_text(
        "hyperparameters",
